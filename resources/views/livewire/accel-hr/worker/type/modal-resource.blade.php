...
new class extends Component {
    ...
    public function save()
    {
        ...
        LivewireAlert::title('')
            ->text('Berhasil ' . (is_null($this->type_id) ? 'menambah' : 'mengubah') . ' Jenis Pekerja')
            ->success()
            ->toast()
            ->position('bottom-end')
            ->show();
    ...
    public function ask_to_delete_type($type_id)
    {
        ...
        LivewireAlert::title('Peringatan')
            ->text('Perintah ini akan menghapus jenis pekerja dengan nama '.$type->name.', Anda yakin ingin melanjutkan?')
            ->asConfirm()
    ...
    public function delete_type()
    {
        ...
        if($type) {
            ...
            LivewireAlert::title('')
            ->text('Berhasil menghapus jenis pekerja')
            ->success()
            ->toast()
            ->position('bottom-end')
            ->show();
        ...
        <div class="modal-content">
            ...
            <div wire:loading.remove wire:target="set_type, reset_type, save" class="modal-body pt-0">
                <div class="text-center mb-4">
                    <h3 class="mb-0">{{ is_null($type_id) ? 'Tambah' : 'Edit' }} Jenis Pekerja</h3>
                    <p>Di sini, Anda dapat {{ is_null($type_id) ? 'menambah data' : 'mengubah informasi' }} jenis pekerja.</p>
                </div>
